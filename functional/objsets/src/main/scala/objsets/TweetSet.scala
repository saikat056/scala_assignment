...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    s = left.filterAcc(p, s)
    s = right.filterAcc(p, s)
    s
  }

  def mostRetweeted : Tweet = {
     var mostRe: Tweet = elem
     val mostLeft : Tweet = {
       if (left != null)
         left.mostRetweeted
       else
     ...
     if(mostRight != null){
       mostRe =
         if(elem.retweets >= mostRight.retweets)
           elem
         else
           mostRight
     }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    var tweets : TweetSet = new Empty
    for(key <- google) {
      tweets = tweets.union(allTweets.filter(tw => (tw.text.contains(key))))
    }
    tweets
  }
