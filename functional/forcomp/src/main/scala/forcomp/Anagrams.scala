...
object Anagrams {
  ...
  def wordAnagrams(word: Word): List[Word] = for(w <- dictionary; if wordOccurrences(word) == wordOccurrences(w)) yield w
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val map1 = x.foldLeft(Map.empty[Char, Int])( (map, value) => map + (value._1 -> value._2) )
    val map2 = y.foldLeft(map1)( (map, value) =>
        { val v : Int = map.apply(value._1)
          if (v == value._2)
            map - (value._1)
          else
